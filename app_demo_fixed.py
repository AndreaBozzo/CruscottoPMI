...
# Cruscotto Finanziario per PMI – Build aggiornata con fix demo
...
# PROCESSA I DATI (comune a demo e reali)
for (azienda, anno), dfs in bilanci.items():
    row = calcola_kpi(dfs["ce"], dfs["attivo"], dfs["passivo"], benchmark)
    if "Errore" in row:
        st.warning(f"Errore su {azienda} {anno}: {row['Errore']}")
        continue
    row.update({"Azienda": azienda, "Anno": str(int(anno))})
    tabella_kpi.append(row)
    tabella_voci.append({
        "Azienda": azienda, "Anno": str(int(anno)),
