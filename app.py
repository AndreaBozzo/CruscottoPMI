...
# ────────── CARICAMENTO DATI REALI (non‐demo) ──────────
...
# ────────── MODALITÀ DEMO ──────────
if demo_mode:
    st.info("Modalità demo attiva: vengono caricati dati di esempio.")
    bilanci = {
        ("Alpha Srl", 2022): {"ce": pd.DataFrame({
            ...
            "passivo":pd.DataFrame({"Passività e Patrimonio Netto":["Debiti a breve","Patrimonio netto"],
                                     "Importo (€)":[85_000,420_000]})},
        ("Beta Spa", 2022): {"ce": pd.DataFrame({
                "Voce":["Ricavi","Utile netto","EBIT","Spese operative"],
                "Importo (€)":[1_750_000,120_000,130_000,260_000]}),
            "attivo":pd.DataFrame({"Attività":["Disponibilità liquide"],"Importo (€)":[150_000]}),
            "passivo":pd.DataFrame({"Passività e Patrimonio Netto":["Debiti a breve","Patrimonio netto"],
                                     "Importo (€)":[120_000,500_000]})}
    }
...
for (azienda, anno), d in sorted(bilanci.items()):
    try:
        ...
        eda_marg     = round(ebitda / ricavi * 100,2)
        roe          = round(utile_netto / patrimonio * 100,2)
        roi          = round(ebit / totale_att * 100,2)
        curr_ratio   = round(liquidita / debiti_brevi,2)

        valutazione = "Ottima solidità ✅"
        if any([eda_marg<10, roe<5, roi<5, curr_ratio<1]): valutazione = "⚠️ Alcuni indici critici"
        if all([eda_marg<10, roe<5, roi<5, curr_ratio<1]): valutazione = "❌ Situazione critica"

        indice = round(((eda_marg/benchmark["EBITDA Margin"]+roe/benchmark["ROE"]+
                         roi/benchmark["ROI"]+curr_ratio/benchmark["Current Ratio"])/4)*10,1)

        tabella_kpi.append({
            "Azienda":azienda,"Anno":int(anno),
            "EBITDA Margin":eda_marg,"Benchmark EBITDA":benchmark["EBITDA Margin"],
            "Δ EBITDA": eda_marg-benchmark["EBITDA Margin"],
            "ROE":roe,"Benchmark ROE":benchmark["ROE"],"Δ ROE": roe-benchmark["ROE"],
            "ROI":roi,"Benchmark ROI":benchmark["ROI"],"Δ ROI": roi-benchmark["ROI"],
            "Current Ratio":curr_ratio,"Benchmark Current":benchmark["Current Ratio"],
            "Δ Current":curr_ratio-benchmark["Current Ratio"],
            "Indice Sintetico":indice,"Valutazione":valutazione,"Ricavi":ricavi
        })
...
if tabella_kpi:
    ...
    st.dataframe(
        df_kpi.style.format("{:.2f}", na_rep="-").apply(evid, axis=1),
    ...
    st.dataframe(yy, use_container_width=True)
    ...
    with pd.ExcelWriter(buf_x,engine="xlsxwriter") as w:
        df_kpi.to_excel(w,"KPI",index=False); yy.to_excel(w,"Δ_YoY",index=False)
    st.download_button("📥 Excel",buf_x.getvalue(),"report_finanziario.xlsx")

    def pdf(df,logo="A_logo_for_Andrea_Bozzo_is_depicted_in_the_image,_.png"):
        b=BytesIO();c=canvas.Canvas(b,pagesize=A4);w,h=A4
        if os.path.exists(logo): c.drawImage(logo,2*cm,h-3.5*cm,3*cm,3*cm,preserveAspectRatio=True,mask='auto')
        ...
        for _,r in df.sort_values("Anno").iterrows():
            ...
            y-=0.4*cm
            if y<5*cm:c.setFont("Helvetica-Oblique",8);c.drawString(2*cm,2*cm,"© 2025 Andrea Bozzo – Cruscotto PMI")
            if y<5*cm:c.showPage();y=h-4.5*cm;c.setFont("Helvetica",11)
        c.save();b.seek(0);return b
    st.download_button("📄 PDF",pdf(df_kpi),"report_finanziario.pdf")
